...
public final class FtfCellCacheProbePack implements ProbePack {
    ...
    public void register() {
        ProbeRegistry.register("squinch:ftf-tile-cache", "1", request -> server -> {
            JsonObject config = request.config();
            if (!config.has("coordinates") || !config.get("coordinates").isJsonArray()) {
                throw new IllegalArgumentException("ftf-tile-cache requires coordinates");
            }
            ...
            if (context == null || context.cache == null) {
                return ProbeResult.partial(
                    ProbePhase.GENERATION, new JsonObject(), 0, coordinates.size(),
                    "ftf-generator-context-or-tile-cache-unavailable"
                );
            }
            ...
            data.addProperty("authority", "ftf-horizontal-cell-model");
    ...
    private static List<String> fields(JsonObject config) {
        ...
        for (var element : config.getAsJsonArray("fields")) {
            ...
            if (!FIELDS.contains(field)) {
                throw new IllegalArgumentException("unknown FTF cell field: " + field);
            }
            result.add(field);
        }
        return result;
    ...
    private static JsonObject sample(
        ...
        for (String field : fields) {
            switch (field) {
                ...
                default -> throw new IllegalArgumentException("unknown FTF cell field: " + field);
            }
        }
        return result;
    }
