...
public final class CellScanMain {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            HolderLookup.Provider provider = new RegistrySetBuilder()
                .add(FTFRegistries.NOISE, context -> PresetNoiseData.bootstrap(preset, context))
                .build(builtIns);
    ...
    private static void clearLegacyCaches() throws Exception {
        try {
            Class<?> manager = Class.forName(
                "etcodehome.freeterraforged.concurrent.cache.CacheManager"
            );
    ...
    private static String authority(String mode) {
        return mode.equals("tile") ? "ftf-horizontal-cell-model" : "prediction";
    }
