...
The focused FTF diagnostic run `20260828T072705Z-04aa61a424` installs `{}` as an existing config. It
independently reports 25 distinct duplicate placements without an observer failure, including
adjacent copies of `biomeswevegone:vanilla/flower_default` in `minecraft:windswept_savanna`,
immediately before Minecraft reports that biome's feature-order cycle. The primary BWG graph and
spatial scenarios instead declare that generated config absent and restore the prior file after the
run, so their default first-start measurements are repeatable.
