...
| `fix/biome-climate-mapping`        | Yes                           | Clean PR/ship candidate with no QA scaffolding: root mapping fix `bbd845c`, dynamic-banding commit `7a0491b` (the clean cherry-pick of `ad491ab`), and cached-chunk-sampler live-path fix `8d0dea5` (the clean cherry-pick of `366caae`). Merge commit `032e63b` integrates `upstream/1.21.1` through `9e445dd`; both loader builds pass. As the QA branch gains further work, replicate only production commits here.                                                                                                                              |
