...
| `qa/biome-climate-mapping`         | Yes                           | Working branch for the underground biome-climate-banding line. Root mapping fix `bbd845c`, production dynamic-banding commit `ad491ab`, cached-chunk-sampler live-path fix `366caae`, standalone scanner `78fab7e`, region-aware scanner baseline `0664f23`, and authoritative finished-chunk scanner `d2b50c3`. Both real fixes and QA/testing instrumentation should continue landing here. See `plans/biome-climate-banding/biome-climate-banding-investigation.md` and `plans/biome-climate-banding/dynamic-underground-biome-banding-plan.md`. |
| `fix/biome-climate-mapping`        | Yes                           | Clean PR/ship candidate with no QA scaffolding: root mapping fix `a5bee8f` (message-rewritten equivalent of QA commit `bbd845c`), dynamic-banding commit `622c56a` (equivalent of `ad491ab`), and cached-chunk-sampler live-path fix `9a1ccdb` (equivalent of `366caae`). Merge commit `9099214` integrates `upstream/1.21.1` through `9e445dd`; both loader builds pass. As the QA branch gains further work, replicate only production commits here.                                                                                              |
