# FreeTerraForged Engineering Wiki

Current FTF-specific concepts that would otherwise require reconstructing behavior from source.
Pages describe mental models, data flow, coordinate meanings, and compatibility invariants. They do
not contain investigations, QA runs, branch state, or feature history.

## Concepts

- [Terrain coordinates and levels](concepts/terrain-coordinates-and-levels.md)
- [Terrain shaping and region selection](concepts/terrain-shaping-and-regions.md)
- [Cell, tile, and lookup pipeline](concepts/cell-tile-and-lookup-pipeline.md)
- [Biome selection and compatibility runtime](concepts/biome-selection-and-compatibility-runtime.md)
- [Hydrology and shoreline geometry](concepts/hydrology-and-shore-geometry.md)
- [Features and structure placement](concepts/features-and-structure-placement.md)
- [Surface rules and materials](concepts/surface-rules-and-materials.md)
- [Compatibility invariants](concepts/compatibility-invariants.md)

General Minecraft mechanisms live in the [Minecraft reference](../../../wiki/README.md).
Feature-specific decisions and work remaining are recorded in `../plans/` or the relevant PR
description.
