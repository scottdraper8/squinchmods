...
public final class TreeifyYaclStateAccess
{
	private TreeifyYaclStateAccess() {
	}

	public static Optional<TreeifyScreenState> capture(YACLScreen screen) {
		if (!(screen.tabNavigationBar.getTabManager().getCurrentTab() instanceof YACLScreen.CategoryTab categoryTab)) {
			return Optional.empty();
		}

		SearchFieldWidget searchField = readTypedField(categoryTab, "searchField", SearchFieldWidget.class);
		...
		return Optional.of(new TreeifyScreenState(
	...
	public static void restore(YACLScreen screen, TreeifyScreenState state) {
		if (!(screen.tabNavigationBar.getTabManager().getCurrentTab() instanceof YACLScreen.CategoryTab categoryTab)) {
			return;
		}
	...
	private static Map<String, Boolean> readGroupExpansionState(OptionListWidget optionList) {
		...
		for (OptionListWidget.Entry entry : optionList.children()) {
			if (entry instanceof OptionListWidget.GroupSeparatorEntry groupSeparatorEntry) {
				getGroupName(groupSeparatorEntry).ifPresent(groupName ->
	...
	private static void applyGroupExpansionState(OptionListWidget optionList, Map<String, Boolean> collapsedGroups) {
		for (OptionListWidget.Entry entry : optionList.children()) {
			if (entry instanceof OptionListWidget.GroupSeparatorEntry groupSeparatorEntry) {
				getGroupName(groupSeparatorEntry).ifPresent(groupName -> {
	...
	private static Optional<String> getGroupName(OptionListWidget.GroupSeparatorEntry groupSeparatorEntry) {
	...
	@Nullable
	private static OptionListWidget getOptionListWidget(Object tab) {
	...
	@Nullable
	private static <T> T readTypedField(Object instance, String name, Class<T> expectedType) {
