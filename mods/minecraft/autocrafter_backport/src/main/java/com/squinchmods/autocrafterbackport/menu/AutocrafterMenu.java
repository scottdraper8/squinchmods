package com.squinchmods.autocrafterbackport.menu;
...
public class AutocrafterMenu extends AbstractContainerMenu {
  ...
  public AutocrafterMenu(
      int containerId, Inventory playerInventory, AutocrafterBlockEntity blockEntity) {
    super(AutocrafterBackportMod.AUTOCRAFTER_MENU.get(), containerId);
    this.blockEntity = blockEntity;
    this.enabledData =
        new ContainerData() {
          @Override
          public int get(int index) {
            return AutocrafterMenu.this.blockEntity.isSlotEnabled(index) ? 1 : 0;
          }

          @Override
          public void set(int index, int value) {
            AutocrafterMenu.this.blockEntity.setSlotEnabled(index, value != 0);
          }
    ...
    for (int row = 0; row < 3; row++) {
      for (int col = 0; col < 3; col++) {
        ...
        int x = 26 + col * 18;
        int y = 17 + row * 18;
        this.addSlot(new AutocrafterMenu.AutocrafterInputSlot(blockEntity, index, x, y));
      }
    }

    this.addSlot(new AutocrafterMenu.AutocrafterOutputSlot(blockEntity, 134, 35));
    int invStartY = 84;
  ...
  public AutocrafterMenu(int containerId, Inventory playerInventory, FriendlyByteBuf data) {
    this(
        containerId,
        playerInventory,
  ...
  private static class AutocrafterContainer implements Container {
    private final AutocrafterBlockEntity be;

    private AutocrafterContainer(AutocrafterBlockEntity be) {
      this.be = be;
    }
  ...
  private static class AutocrafterInputSlot extends Slot {
    private final AutocrafterBlockEntity be;
    private final int slotIndex;

    public AutocrafterInputSlot(AutocrafterBlockEntity be, int index, int x, int y) {
      super(new AutocrafterMenu.AutocrafterContainer(be), index, x, y);
      this.be = be;
  ...
  private static class AutocrafterOutputSlot extends Slot {
    private final AutocrafterBlockEntity be;

    public AutocrafterOutputSlot(AutocrafterBlockEntity be, int x, int y) {
      super(new AutocrafterMenu.AutocrafterContainer(be), 0, x, y);
      this.be = be;
    }
