...
public class BiomePatchService {
    ...
    public PatchDecision planPatch(ResourceLocation biomeId, ResourceLocation featureId, Holder<PlacedFeature> original) {
    ...
    private PlacedFeature pruneMushroomsFromRandomSelector(PlacedFeature original) {
        RandomFeatureConfiguration config = (RandomFeatureConfiguration) original.feature().value().config();

        List<WeightedPlacedFeature> filtered = config.features.stream()
            .filter(w -> !isMushroomFeature(w.feature.value()))
            .toList();
        ...
        ConfiguredFeature<?, ?> newCf = new ConfiguredFeature<>(
