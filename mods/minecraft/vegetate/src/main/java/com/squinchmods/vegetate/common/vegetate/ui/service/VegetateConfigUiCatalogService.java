...
public final class VegetateConfigUiCatalogService implements ConfigUiCatalogService {
  ...
  public VegetateConfigUiCatalogService(
      VegetateConfigService configService, BiomeVegetationIndex index) {
  ...
  public ConfigUiCatalogSnapshot getCatalogSnapshot() {
    Map<VegetationCategory, List<ConfigUiEntryView>> categorized =
        new EnumMap<>(VegetationCategory.class);
    ...
    for (Map.Entry<String, VegetationFeatureRule> entry :
        configService.getConfig().getFeatureRules().entrySet()) {
      if (seenIds.add(entry.getKey())) {
        VegetationCategory category =
            VegetationCategoryResolver.resolve(
                entry.getKey(), entry.getValue().getCategory(), null);
    ...
    return new ConfigUiCatalogSnapshot(
        Component.translatable("vegetate.gui.title"), categories, Optional.empty());
  }
