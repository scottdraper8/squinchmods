...
public final class VegetationWorldgenDataProvider {
  public static BiomeVegetationIndex discover() {
                  ...
                  placedFeatureHolder
                      ...
                      .ifPresent(
                          placedFeatureKey -> {
                            ...
                            featureToBiomes
                                .computeIfAbsent(featureId, k -> new HashSet<>())
                                .add(biomeKey);
                          });
                }
              }
    ...
    featureToBiomes.forEach(
        (featureId, biomes) -> {
          placedFeatureRegistry
              .get(ResourceKey.create(Registries.PLACED_FEATURE, featureId))
              .ifPresent(
                  placedFeatureHolder -> {
                    ...
                    if (VegetationCategoryResolver.isMushroomCategory(
                        category, featureId.toString())) {
                      index.addEntry(
                          new DiscoveredVegetationEntry(
                              featureId, category, biomes, decorationStep));
                    }
                  });
        });
