...
public final class VegetateConfigScreen
{
	...
	private void composeMushroomPage(YetAnotherConfigLib.Builder yaclBuilder, ConfigUiCatalogSnapshot snapshot) {
			...
			for (ConfigUiEntryView entryView : categoryView.entries()) {
				...
				.description(OptionDescription.createBuilder()
					...
					.controller(opt -> BooleanControllerBuilder.create(opt)
						.formatValue(val -> val
							? Component.translatable("vegetate.gui.label.enabled").withStyle(ChatFormatting.GREEN)
							: Component.translatable("vegetate.gui.label.disabled").withStyle(ChatFormatting.RED))
						.coloured(true))
					.build();
		...
		var globalGroup = OptionGroup.createBuilder()
			.name(Component.translatable("vegetate.gui.group.global"));

		var globalToggle = Option.<Boolean>createBuilder()
			.name(Component.translatable("vegetate.gui.option.mushroom_worldgen_global"))
		.description(OptionDescription.createBuilder()
			.text(Component.translatable("vegetate.gui.option.mushroom_worldgen_global.description"))
			.build())
			.binding(
				true,
